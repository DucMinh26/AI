...
def respond_chat(user,message):
    ...
    is_important = should_remember(msg_clean)

    if not msg_clean:
        return "Bạn chưa nói gì cả!"
    
    if is_important:

        reply = f"Mình đã nhớ điều đấy"
    ...
    if random.random() < 0.2:
        ...
        if user['profile']['likes']:
            memories.append((random.choice(user['profile']['likes']), "Mình nhớ bạn nói là {0}"))
        if user['profile']['dislikes']:
            memories.append((random.choice(user['profile']['dislikes']), "Mình nhớ bạn nói là {0}"))
        if user['profile']['goals']:
            memories.append((random.choice(user['profile']['goals']), "Mình nhớ bạn nói là {0}"))
        if memories:
            content, template = random.choice(memories)
    ...
    if "tên" in message.strip().lower():
        ...
        last_ai_msg = [msg for msg in reversed(memory) if msg['role'] == 'AI']

        if last_ai_msg:
            reply = f"Như mình đã nói trước đó, {last_ai_msg[0]['text']}"
...
def should_remember(message):
    keyword = ['thích','yêu',
               'ghét','không thích',
               'muốn','ước','mục tiêu','dự định','kế hoạch']
    
    msg = message.strip().lower()
    return any(k in msg for k in keyword)

def update_profile(user, message):
    msg = message.strip().lower()
    update = True

    if "thích" in msg or "yêu" in msg:
        user['profile']['likes'].append(message)
    
    elif "ghét" in msg or "không thích" in msg:
        user['profile']['dislikes'].append(message)

    elif "mục tiêu" in msg or "ước" in msg or "muốn" in msg or "dự định" in msg or "kế hoạch" in msg:
        user['profile']['goals'].append(message)
